...
namespace Steamp3.UI
{
    ...
    public class Achievement : GenericListItem
    {
        ...
        #region Objects
        private string p_ID, p_Name, p_Description;
        ...
        internal int Value
        {
            get
            {
                switch (p_ID)
                {
                    ...
                    case "rs":
                        return Global.Stats.RadioStar;
                    case "rp":
                        return Global.Stats.Repetition;
                    case "sh":
                        return Global.Stats.ShamelessPlug;
                    case "ss":
                        return Global.Stats.ShortStop;
                    case "sd":
                        return Global.Stats.SkinnyDipper;
                    default:
                        return 0;
                }
            }
            set
            {
                ...
                switch (p_ID)
                {
                    ...
                    case "mp":
                        ...
                        break;
                    case "rs":
                        Global.Stats.RadioStar = value;
                        break;
                    case "rp":
                        Global.Stats.Repetition = value;
                        break;
                    case "sh":
                        Global.Stats.ShamelessPlug = value;
                        break;
                    case "ss":
                        Global.Stats.ShortStop = value;
                        break;
                    case "sd":
                        Global.Stats.SkinnyDipper = value;
                        break;
                }

                if (value.Equals(Maximum)) Global.Steam.SendStatus("Achievement earned", p_Name, true);
            }
        }
        #endregion
        ...
        internal string Save()
        {
            ...
            if (fullOnly)
            {
                if (Value.Equals(Maximum)) result += Value.ToString();
                else result += "0";
            }
    ...
    public class AchievementList : GenericList<Achievement>
    {
        ...
        private UI.Station p_RadioStarStation;
        private int p_RadioStarSeconds;
        private string p_RepetitionURL;
        private List<SteamAPI.SteamID> p_ShamelessPlugIDs;
        #endregion
        ...
        public AchievementList(Window owner, Control parent) : base(owner, parent)
        {
            ...
            p_RadioStarStation = null;
            p_RadioStarSeconds = 0;
            p_RepetitionURL = string.Empty;
            p_ShamelessPlugIDs = new List<SteamAPI.SteamID>();
        }

        public override void Dispose()
        {
            p_ShamelessPlugIDs.Clear();
            p_RepetitionURL = string.Empty;
            p_RadioStarSeconds = 0;
            p_RadioStarStation = null;
        ...
        {
            Achievement item = e.Item as Achievement;
            if (item == null) return;

            int x = e.Bounds.Width / 3;
            ...
            if (item.Equals(SelectedItem))
            {
                ...
                Rectangle r = new Rectangle(e.Bounds.X, e.Bounds.Y, e.Bounds.Width - 100, e.Bounds.Height);
            ...
            if (item.Value.Equals(item.Maximum)) bgColor = Global.Skin.List.Play_BGColor;

            e.Graphics.DrawString(item.Name, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);
            e.Graphics.DrawString(item.Description, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X + x, e.Bounds.Y, (x * 2) - 100, e.Bounds.Height), e.StringFormat);

            if (item.Value > 0) Global.FillRoundedRectangle(e.Graphics, new Rectangle((e.Bounds.X + e.Bounds.Width) - 100, e.Bounds.Y + 1, Global.GetPercent(item.Value, item.Maximum, 100), e.Bounds.Height - 2), new SolidBrush(bgColor));
            Global.DrawRoundedRectangle(e.Graphics, new Rectangle((e.Bounds.X + e.Bounds.Width) - 100, e.Bounds.Y + 1, 100, e.Bounds.Height - 2), new Pen(Global.Skin.List.BorderColor));

            e.StringFormat.Alignment = StringAlignment.Center;
            e.Graphics.DrawString(item.Value.ToString() + " / " + item.Maximum.ToString(), Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 100, e.Bounds.Y, 100, e.Bounds.Height), e.StringFormat);
        ...
        public Achievement GetItemByID(string id)
        {
            foreach (Achievement item in Items)
            {
                if (item.ID.ToLower().Equals(id.ToLower())) return item;
            }

            return null;
        }

        internal void IncreaseChattyCathy(SteamAPI.SteamID chatID)
        {
            Achievement achievement = GetItemByID("cc");
        ...
        internal void IncreaseJukeboxHero(PlaylistItem item)
        {
            Achievement achievement = GetItemByID("jh");
        ...
        internal void IncreaseLeetHaxor(UI.PlaylistItem item)
        {
            Achievement achievement = GetItemByID("lh");
        ...
        internal void IncreaseMP3Playah(UI.PlaylistItem item)
        {
            Achievement achievement = GetItemByID("mp");
        ...
        internal void IncreaseRadioStar(UI.Station station)
        {
            Achievement achievement = GetItemByID("rs");

            if (achievement != null && station != null)
            {
                if (station.Equals(p_RadioStarStation))
                {
                    if (p_RadioStarSeconds < 3600) p_RadioStarSeconds++;
                    else
                    {
                        achievement.Value++;
                        ReDraw();
                ...
                {
                    p_RadioStarStation = station;
                    p_RadioStarSeconds = 0;
                }
            }
        }

        internal void IncreaseRepetition(UI.PlaylistItem item, string status)
        {
            Achievement achievement = GetItemByID("rp");
        ...
        internal void IncreaseShamelessPlug(SteamAPI.SteamID friendID)
        {
            Achievement achievement = GetItemByID("sh");

            foreach (SteamAPI.SteamID id in p_ShamelessPlugIDs)
            {
                if (id.ToUInt64().ToString().Equals(friendID.ToUInt64().ToString())) return;
            }

            p_ShamelessPlugIDs.Add(friendID);

            if (achievement != null && p_ShamelessPlugIDs.Count < 4)
            {
                achievement.Value++;
                ReDraw();
            }
        ...
        internal void IncreaseShortStop(UI.PlaylistItem item)
        {
            Achievement achievement = GetItemByID("ss");

            if (achievement != null && item.Duration.TotalSeconds < 60)
            {
                achievement.Value++;
                ReDraw();
            }
        ...
        internal void IncreaseSkinnyDipper()
        {
            Achievement achievement = GetItemByID("sd");

            if (achievement != null)
            {
                if (Global.MainWindow.SkinList.GetMySkinsCount() <= 5)
                {
                    achievement.Value = Global.MainWindow.SkinList.GetMySkinsCount();
                    ReDraw();
                }
            }
        }
    ...
    public class CommandList : GenericList<Command>
    {
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            Command item = e.Item as Command;
            if (item == null) return;
            ...
            if (string.IsNullOrEmpty(item.Param)) e.Graphics.DrawString(Global.Settings.Trigger + item.ID, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);
            else e.Graphics.DrawString(Global.Settings.Trigger + item.ID + " {" + item.Param + "}", Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);

            e.Graphics.DrawString(FormatCommand(item), Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X + x, e.Bounds.Y, (x * 2) - 30, e.Bounds.Height), e.StringFormat);

            e.StringFormat.Alignment = StringAlignment.Far;
            if (item.New) e.Graphics.DrawString("New!", Skin.WindowFont, new SolidBrush(Global.Skin.List.Play_TextColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 30, e.Bounds.Y, 30, e.Bounds.Height), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Near;
        }
    ...
    {
        #region Objects
        private Graphics p_Graphics;
        private GenericListItem p_Item;
        private Rectangle p_Bounds;
        private StringFormat p_StringFormat;
        #endregion

        #region Properties
        public Graphics Graphics
        {
            get { return p_Graphics; }
        }
        ...
        {
            get { return p_StringFormat; }
        }
        #endregion

        #region Constructor/Destructor
        public DrawItemEventArgs(Graphics graphics, GenericListItem item, Rectangle bounds)
        {
            p_Graphics = graphics;
            p_Item = item;
            p_Bounds = bounds;

            p_StringFormat = (StringFormat)StringFormat.GenericTypographic.Clone();
            p_StringFormat.Alignment = StringAlignment.Near;
            //p_StringFormat.FormatFlags = StringFormatFlags.LineLimit | StringFormatFlags.FitBlackBox;
            p_StringFormat.LineAlignment = StringAlignment.Center;
            p_StringFormat.Trimming = StringTrimming.EllipsisCharacter;
        }

        public virtual void Dispose()
        {
            p_StringFormat.Dispose();
            p_Graphics = null;
            p_Item = null;
            p_Bounds = new Rectangle();
        }
        #endregion
    }
    #endregion
    ...
    public class FilterBar : Control
    {
        ...
        protected override void OnBoundsChanged(EventArgs e)
        {
            ...
            p_DropDown.SetBounds(Bounds.X + (Bounds.Width - 151), Bounds.Y + 1, 150, Bounds.Height - 2);
    ...
    public class GenericList<T> : Control where T : GenericListItem
    {
        ...
        protected override void OnMouseWheel(MouseEventArgs e)
        {
            ...
            if (e.Delta < 0) FirstIndex += SystemInformation.MouseWheelScrollLines;
            else FirstIndex -= SystemInformation.MouseWheelScrollLines;
        ...
        #region Public Methods
        public T SelectItemByText(string text)
        {
            foreach (T item in FilteredItems)
            {
                if (item.Text.ToLower().Equals(text.ToLower()))
                {
                    SelectedItem = item;
                    return item;
                }
        ...
        {
            DrawItemEventArgs e = new DrawItemEventArgs(g, null, new Rectangle());
            int x = (bounds.Width - p_ScrollBar.Bounds.Width) - 10;
            int y = 0;

            if (p_Refreshing) g.DrawString("Refreshing, please wait...", Skin.WindowFont, new SolidBrush(Global.Skin.List.TextColor), new RectangleF(bounds.X + 3, bounds.Y + (bounds.Height / 2) - (p_ItemHeight / 2), x, p_ItemHeight), e.StringFormat);
            else if (p_FilteredItems.Count == 0) g.DrawString(p_Mask, Skin.WindowFont, new SolidBrush(Global.Skin.List.TextColor), new RectangleF(bounds.X + 3, bounds.Y + (bounds.Height / 2) - (p_ItemHeight / 2), x, p_ItemHeight), e.StringFormat);
            else
            {
                for (int i = FirstIndex; i < p_FilteredItems.Count; i++)
                {
                    ...
                    e = new DrawItemEventArgs(g, p_FilteredItems[i], new Rectangle(bounds.X + 3, (bounds.Y + y) + 3, x, p_ItemHeight));
                    DrawItem(e);
        ...
        protected virtual void DrawItem(DrawItemEventArgs e)
        {
            Color textColor = Global.Skin.List.TextColor;

            if (e.Item.Equals(SelectedItem))
            {
                textColor = Global.Skin.List.Down_TextColor;
            ...
            if (e.Item.Equals(PlayingItem)) textColor = Global.Skin.List.Play_TextColor;
            if (e.Item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.Graphics.DrawString(e.Item.ToString(), Skin.WindowFont, new SolidBrush(textColor), e.Bounds, e.StringFormat);
        }
    ...
    public class Library : GenericList<LibraryItem>
    {
        ...
        {
            p_FilterBar = new UI.FilterBar(owner, parent, "Search Library", new string[] { "All Playlists (0)", "Foobar Playlists (0)", "iTunes Playlists (0)", "STEAMp3 Playlists (0)", "Winamp Playlists (0)", "WMP Playlists (0)", "Generic Playlists (0)" }, "Filter Library");
            p_FilterBar.SelectedIndexChanged += new EventHandler(p_FilterBar_SelectedIndexChanged);
            p_FilterBar.TextChanged += new EventHandler(p_FilterBar_TextChanged);
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            LibraryItem item = e.Item as LibraryItem;
            if (item == null) return;
            ...
            if (item.URL.ToLower().Equals(Global.MainWindow.Playlist.URL.ToLower())) textColor = Global.Skin.List.Play_TextColor;
            if (item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.Graphics.DrawString(item.Text, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, e.Bounds.Width - 110, e.Bounds.Height), e.StringFormat);

            e.StringFormat.Alignment = StringAlignment.Far;
            e.Graphics.DrawString(item.PlaylistType.ToString(), Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 110, e.Bounds.Y, 110, e.Bounds.Height), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Near;
        }
        ...
        protected override void FilterItems()
        {
            ...
            switch ((FilterTypes)p_FilterBar.DropDown.SelectedIndex)
            {
                case FilterTypes.AllPlaylists:
                    FilteredItems.AddRange(items);
                    break;
                case FilterTypes.FoobarPlaylists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".fpl")) FilteredItems.Add(item);
                    }
                    break;
                case FilterTypes.ItunesPlaylists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".itl")) FilteredItems.Add(item);
                    }
                    break;
                case FilterTypes.STEAMp3Playlists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".sp3")) FilteredItems.Add(item);
                    }
                    break;
                case FilterTypes.WinampPlaylists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".m3u")) FilteredItems.Add(item);
                    }
                    break;
                case FilterTypes.WMPPlaylists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".wpl")) FilteredItems.Add(item);
                    }
                    break;
                case FilterTypes.GenericPlaylists:
                    foreach (LibraryItem item in items)
                    {
                        if (item.URL.ToLower().EndsWith(".pls")) FilteredItems.Add(item);
                    }
                    break;
            }
        ...
        protected override void OnFilteredItemsChanged(EventArgs e)
        {
            ...
            p_FilterBar.DropDown.Items[1] = "Foobar Playlists (" + Global.FormatNumber(GetFoobarPlaylistsCount()) + ")";
            p_FilterBar.DropDown.Items[2] = "iTunes Playlists (" + Global.FormatNumber(GetItunesPlaylistsCount()) + ")";
            p_FilterBar.DropDown.Items[3] = "STEAMp3 Playlists (" + Global.FormatNumber(GetSteamp3PlaylistsCount()) + ")";
            p_FilterBar.DropDown.Items[4] = "Winamp Playlists (" + Global.FormatNumber(GetWinampPlaylistsCount()) + ")";
            p_FilterBar.DropDown.Items[5] = "WMP Playlists (" + Global.FormatNumber(GetWMPPlaylistsCount()) + ")";
            p_FilterBar.DropDown.Items[6] = "Generic Playlists (" + Global.FormatNumber(GetGenericPlaylistsCount()) + ")";
            p_FilterBar.DropDown.Text = p_FilterBar.DropDown.Items[p_FilterBar.DropDown.SelectedIndex];
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            {
                Global.MainWindow.Playlist.Load(SelectedItem);
            }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                {
                    p_ContextMenu.AddRange(new MenuItem[] { new MenuItem("", "Load Playlist"), new UI.MenuItem("Í", "Rename Playlist..."), new UI.MenuItem("x", "Delete Playlist"), new MenuItem(string.Empty, "-") });
                }
                p_ContextMenu.AddRange(new MenuItem[] { new MenuItem("q", "Refresh Library"), new MenuItem(string.Empty, "-"), new UI.MenuItem("", "New Playlist...") });
            ...
            switch (p_ContextMenu.SelectedItem.Text)
            {
                ...
                case "Rename Playlist...":
                    ...
                    if (id2.ShowDialog(Owner) == DialogResult.OK)
                    {
                        ...
                        {
                            id = new InfoDialog("Invalid filename, please try again.");
                            id.ShowDialog(Owner);
                            id.Dispose();
                        }
                    }
                ...
                case "Delete Playlist":
                    id = new InfoDialog("Are you sure you want to send this playlist to the recycle bin?" + Environment.NewLine + Environment.NewLine + Path.GetFileName(SelectedItem.URL), InfoDialog.InfoButtons.YesNo);
        ...
        #region Public Methods
        public int GetFoobarPlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".fpl")) result++;
            }

            return result;
        }

        public int GetItunesPlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".itl")) result++;
            }

            return result;
        }

        public int GetSteamp3PlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".sp3")) result++;
            }

            return result;
        }

        public int GetWinampPlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".m3u")) result++;
            }

            return result;
        }

        public int GetWMPPlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".wpl")) result++;
            }

            return result;
        }

        public int GetGenericPlaylistsCount()
        {
            int result = 0;

            foreach (LibraryItem item in Items)
            {
                if (item.URL.ToLower().EndsWith(".pls")) result++;
            }
        ...
        public void Refresh()
        {
            ...
            try
            {
                DirectoryInfo dirInfo = new DirectoryInfo(Global.Settings.MusicFolder);

                FileInfo[] fileInfo = dirInfo.GetFiles();
                foreach (FileInfo fi in fileInfo)
                {
                    string[] filter = new string[] { ".fpl", ".itl", ".sp3", ".m3u", ".wpl", ".pls" };

                    foreach (string ext in filter)
                    {
                        if (fi.FullName.ToLower().EndsWith(ext.ToLower()))
                        {
                            Items.Add(new LibraryItem(fi.FullName));
                            break;
                        }
                    }
                }
        ...
        #region Private Methods
        private void Refresh(DirectoryInfo dirInfo, int level = 1)
        {
            try
            {
                FileInfo[] fileInfo = dirInfo.GetFiles();
    ...
    public class LibraryItem : GenericListItem
    {
        ...
        private PlaylistTypes p_PlaylistType;
        #endregion
        ...
        {
            get { return p_PlaylistType; }
        }
        #endregion

        #region Constructor/Destructor
        public LibraryItem(string url) : base()
        {
            URL = url;
            p_PlaylistType = PlaylistTypes.Unknown;

            if (p_URL.ToLower().EndsWith(".fpl")) p_PlaylistType = PlaylistTypes.Foobar;
            else if (p_URL.ToLower().EndsWith(".itl")) p_PlaylistType = PlaylistTypes.iTunes;
            else if (p_URL.ToLower().EndsWith(".sp3")) p_PlaylistType = PlaylistTypes.STEAMp3;
            else if (p_URL.ToLower().EndsWith(".m3u")) p_PlaylistType = PlaylistTypes.Winamp;
            else if (p_URL.ToLower().EndsWith(".wpl")) p_PlaylistType = PlaylistTypes.WMP;
            else if (p_URL.ToLower().EndsWith(".pls")) p_PlaylistType = PlaylistTypes.Generic;
        }
    ...
    public class Playlist : GenericList<PlaylistItem>
    {
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            PlaylistItem item = e.Item as PlaylistItem;
            if (item == null) return;

            int x = e.Bounds.Width / 2;
            ...
            if (item.Equals(SelectedItem))
            {
                ...
                Rectangle r = new Rectangle(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height);
                LinearGradientBrush b = new LinearGradientBrush(r, Global.Skin.List.Down_BGColor, Color.Transparent, LinearGradientMode.Horizontal);
                b.WrapMode = WrapMode.TileFlipX;
            ...
            if ((int)item.Error != 0) textColor = Global.Skin.List.Stop_TextColor;
            if (item.Equals(PlayingItem)) textColor = Global.Skin.List.Play_TextColor;
            if (item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.Graphics.DrawString(item.Text, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);
            //Global.DrawString(Owner.DeviceContext, item.Text, new Rectangle(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), TextFormatFlags.Left | TextFormatFlags.NoPadding | TextFormatFlags.EndEllipsis);
        }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && Clicks > 1)
            {
                if (Global.Settings.Notifications) Global.MediaPlayer.Play(SelectedIndex, "Playing", true);
                else Global.MediaPlayer.Play(SelectedIndex, string.Empty, true);
            }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (SelectedItem != null) p_ContextMenu.Add(new UI.MenuItem("4", "Play"));
                if (Global.MediaPlayer.IsPlaying(true)) p_ContextMenu.AddRange(new MenuItem[] { new UI.MenuItem(";", "Pause"), new UI.MenuItem("<", "Stop") });
                p_ContextMenu.AddRange(new MenuItem[] { new MenuItem("7", "Previous"), new MenuItem("8", "Next"), new MenuItem("s", "Random") });
                if (SelectedItem != null) p_ContextMenu.AddRange(new MenuItem[] { new MenuItem(string.Empty, "-"), new UI.MenuItem("Í", "Rename File..."), new UI.MenuItem("x", "Delete File"), new UI.MenuItem("Y", "Add to Favorites") });
                if (p_Folders.Count > 0) p_ContextMenu.AddRange(new MenuItem[] { new MenuItem(string.Empty, "-"), new MenuItem("q", "Refresh Playlist") });
        ...
        private void p_PlayButton_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                case "4":
                    if (Global.MediaPlayer.IsPaused())
                    {
                        if (Global.Settings.Notifications) Global.MediaPlayer.Resume("Resumed", true);
                        else Global.MediaPlayer.Resume(string.Empty, true);
                    }
                    else
                    {
                        if (Global.Settings.Notifications) Global.MediaPlayer.Play("Playing", true);
                        else Global.MediaPlayer.Play(string.Empty, true);
                    }
                    break;
                case ";":
                    if (Global.MediaPlayer.IsPlaying(false))
                    {
                        if (Global.Settings.Notifications) Global.MediaPlayer.Pause("Paused", true);
                        else Global.MediaPlayer.Pause(string.Empty, true);
                    }
                    break;
            }
        }
        ...
        private void p_ContextMenu_ItemClicked(object sender, EventArgs e)
        {
            ...
            {
                case "Play":
                    if (Global.Settings.Notifications) Global.MediaPlayer.Play(SelectedItem, "Playing", true);
                    else Global.MediaPlayer.Play(SelectedItem, string.Empty, true);
                    break;
                case "Pause":
                    if (Global.MediaPlayer.IsPlaying(false))
                    {
                    ...
                    else if (Global.Settings.Notifications) Global.Steam.SendError("No file playing", true);
                    break;
                case "Stop":
        ...
        public void AddDirectory(string path)
        {
            ...
            else
            {
                ...
                RefreshDialog rd = new RefreshDialog(folders, "Searching for audio files, please wait...");
                rd.ShowDialog(Owner);

                AddRange(rd.Files);
                //Global.MainWindow.Library.AddRange(rd.Playlists);
        ...
        {
            if (item == null || !File.Exists(item.URL))
            {
                p_URL = Global.Settings.MusicFolder + "\\All Music.sp3";

                AddDirectory(Global.Settings.MusicFolder); //?

                Save(p_URL); //?
                Global.Settings.PlaylistURL = p_URL; //?
            }
            else
            {
                p_URL = item.URL; //?
                ...
                switch (Path.GetExtension(item.URL).ToLower())
                {
                    case ".fpl":
                        LoadFoobarPlaylist(item.URL);
                        break;
                    case ".itl":
                        LoadItunesPlaylist(item.URL);
                        break;
                    case ".sp3":
                        LoadSteamp3Playlist(item.URL);
                        break;
                    case ".m3u":
                        LoadWinampPlaylist(item.URL);
                        break;
                    case ".wpl":
                        LoadWMPPlaylist(item.URL);
                        break;
                    case ".pls":
                        LoadGenericPlaylist(item.URL);
                        break;
                }

                Global.Settings.PlaylistURL = item.URL; //?
                //Global.MainWindow.Library.PlayingItem = item; //?
        ...
        public bool Save(string url)
        {
            try
            {
                ...
                XmlElement root = xml.CreateElement("Steamp3.Playlist");

                foreach (string folder in p_Folders)
                {
                    XmlElement element = xml.CreateElement("Folder");
                    element.SetAttribute("URL", folder);
                    element.SetAttribute("Include", "1");
                    root.AppendChild(element);
                }

                foreach (PlaylistItem item in Items)
                {
                    root.AppendChild(item.ToXml(xml));
                }
        ...
        public void Refresh(bool chat)
        {
            ...
            else
            {
                ...
                RefreshDialog rd = new RefreshDialog(p_Folders, "Refreshing playlist, please wait...");
                rd.ShowDialog(Owner);
                ...
                Items.Clear();
                AddRange(rd.Files);
                //Global.MainWindow.Library.Items.Clear();
                //Global.MainWindow.Library.AddRange(rd.Playlists);

                if (p_URL.ToLower().EndsWith(".sp3")) Save(p_URL); //???

                if (Global.Settings.Notifications) Global.Steam.SendMessage("Playlist refreshed, Files: {ls}" + Global.FormatNumber(Items.Count) + "{rs}" + Environment.NewLine + "Size: {ls}" + Global.ConvertBytes(size) + "{rs} Duration: {ls}" + Global.ConvertTicks(duration, false) + "{rs} Time: {ls}" + Global.ConvertMilliseconds(rd.ElapsedTime, true) + "{rs}", chat);
        ...
        #region Private Methods
        private void LoadFoobarPlaylist(string url)
        {
            // fpl/text
            // works
            ...
            try
            {
                string result = Global.LoadString(url);
                string[] split = result.Split(new string[] { "file://" }, StringSplitOptions.RemoveEmptyEntries);
                ...
                for (int i = 1; i <= split.GetUpperBound(0); i++)
                {
                    ...
                    {
                        if ((int)c > 0) result2 += c.ToString();
                        else break;
                    }

                    if (Global.IsRelativePath(result2)) result2 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(url), result2));

                    foreach (string ext in Global.Settings.MusicFilter.Split(new char[] { ';' }))
                    {
                        if (result2.ToLower().EndsWith(ext.ToLower()))
                        {
                            Items.Add(new PlaylistItem(result2, false));
                            break;
                        }
                    }
                }
        ...
        private void LoadItunesPlaylist(string url)
        {
            // itl & xml
            // works
            ...
            {
                string result = url;
                if (url.ToLower().EndsWith(".itl")) result = Path.GetDirectoryName(url) + "\\" + Path.GetFileNameWithoutExtension(url) + ".xml";

                XmlDocument xml = new XmlDocument();
                xml.Load(result);

                foreach (XmlNode node in xml.SelectNodes("plist/dict/dict/dict"))
                {
                    ...
                    foreach (XmlNode node2 in node.SelectNodes("key"))
                    {
                        switch (node2.InnerText.ToLower())
                        {
                            case "name":
                                title = node2.NextSibling.InnerText;
                                break;
                            case "artist":
                                artist = node2.NextSibling.InnerText;
                                break;
                            case "album":
                                album = node2.NextSibling.InnerText;
                                break;
                            case "genre":
                                genre = node2.NextSibling.InnerText;
                                break;
                            case "kind":
                                type = node2.NextSibling.InnerText;
                                break;
                            case "size":
                                size = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "total time":
                                duration = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "track number":
                                track = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "track count":
                                trackCount = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "year":
                                year = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "bit rate":
                                bitrate = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "sample rate":
                                frequency = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "play count":
                                playCount = Global.StringToInt(node2.NextSibling.InnerText);
                                break;
                            case "location":
                                location = node2.NextSibling.InnerText;
                                break;
                        }
                    }
                    ...
                    {
                        if (Global.IsRelativePath(location)) location = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(url), location));

                        foreach (string ext in Global.Settings.MusicFilter.Split(new char[] { ';' }))
                        {
                            if (location.ToLower().EndsWith(ext.ToLower()))
                            {
                                Items.Add(new PlaylistItem(location, artist, title, album, genre, track, trackCount, year, type, playCount, bitrate, frequency, new TimeSpan(0, 0, 0, 0, duration), size));
                                break;
                            }
                        }
                    }
        ...
        private void LoadSteamp3Playlist(string url)
        {
            // sp3/xml
            // Contains directory(s)
            ...
            try
            {
                ...
                XmlNode root = xml.SelectSingleNode("Steamp3.Playlist");

                XmlNodeList nodeList = root.SelectNodes("File");
                if (nodeList == null || nodeList.Count == 0)
                {
                    foreach (XmlNode node in root.SelectNodes("Folder"))
                    {
                        AddDirectory(Global.GetXmlValue(node, "URL", string.Empty));
                    }
                }
                else
                {
                    foreach (XmlNode node in root.SelectNodes("Folder"))
                    {
                        p_Folders.Add(Global.GetXmlValue(node, "URL", string.Empty));
                    }

                    foreach (XmlNode node in nodeList)
                    {
                        Items.Add(new PlaylistItem(node.OuterXml, true));
                    }
                }
        ...
        private void LoadWinampPlaylist(string url)
        {
            // m3u/text
            // May contain more playlists inside
            // May contain directories
            // works?

            try
            {
                string[] result = Global.LoadArray(url);

                if (result.GetUpperBound(0) < 1) return; //?

                if (result[0].ToLower().StartsWith("#extm3u")) // Extended
                {
                    foreach (string line in result)
                    {
                        if (!line.StartsWith("#"))
                        {
                            ...
                            if (Global.IsDirectory(result2)) AddDirectory(result2); //???
                            else
                            {
                                foreach (string ext in Global.Settings.MusicFilter.Split(new char[] { ';' }))
                                {
        ...
        private void LoadWMPPlaylist(string url)
        {
            // wpl/xml
            // works
            ...
            try
            {
                ...
                foreach (XmlNode node in xml.SelectNodes("smil/body/seq/media"))
                {
                    result = Global.GetXmlValue(node, "src", string.Empty);

                    if (!string.IsNullOrEmpty(result))
                    {
                        if (Global.IsRelativePath(result)) result = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(url), result));

                        foreach (string ext in Global.Settings.MusicFilter.Split(new char[] { ';' }))
                        {
                            if (result.ToLower().EndsWith(ext.ToLower()))
                            {
                                Items.Add(new PlaylistItem(result, false));
                                break;
                            }
                        }
                    }
        ...
        private void LoadGenericPlaylist(string url)
        {
            // pls/ini
            // works
            ...
            try
            {
                ...
                string ini = Global.LoadString(url);
                int count = Global.StringToInt(Global.GetIniValueFromString(ini, "NumberOfEntries", "1"));

                if (count > 1)
                {
                    for (int i = 1; i <= count; i++)
                    {
                        result = Global.GetIniValueFromString(ini, "File" + i.ToString(), string.Empty);
                ...
                {
                    result = Global.GetIniValueFromString(ini, "File1", string.Empty);
    ...
    {
        #region Objects
        private TextBoxGroup p_NameGroup;
        private List p_FolderList;
        private Button p_ButtonAdd, p_ButtonRemove, p_ButtonSave, p_ButtonCancel;
        #endregion

        #region Constructor/Destructor
        public PlaylistDialog() : base()
        {
            MinimumSize = new Size(400, 180);
            Size = new Size(400, 300);
            Text = "STEAMp3 - Playlist Editor";

            p_NameGroup = new TextBoxGroup(this, null, "Playlist name:", "New Playlist");
            p_NameGroup.DefaultButtonClicked += new EventHandler(p_NameGroup_DefaultButtonClicked);
            p_NameGroup.TextChanged += new EventHandler(p_NameGroup_TextChanged);

            p_FolderList = new List(this, null);
            p_FolderList.ItemsChanged += new EventHandler(p_FolderList_ItemsChanged);
            p_FolderList.SelectedItemChanged += new EventHandler(p_FolderList_SelectedItemChanged);

            p_ButtonAdd = new Button(this, null, "Add Folder...");
            p_ButtonAdd.MouseClick += new MouseEventHandler(p_ButtonAdd_MouseClick);

            p_ButtonRemove = new Button(this, null, "Remove Folder");
            p_ButtonRemove.Enabled = false;
            p_ButtonRemove.MouseClick += new MouseEventHandler(p_ButtonRemove_MouseClick);

            p_ButtonSave = new Button(this, null, "Save Playlist");
            p_ButtonSave.Enabled = false;
            p_ButtonSave.MouseClick += new MouseEventHandler(p_ButtonSave_MouseClick);

            p_ButtonCancel = new Button(this, null, "Cancel");
            p_ButtonCancel.MouseClick += new MouseEventHandler(p_ButtonCancel_MouseClick);
        }
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            p_NameGroup.SetBounds(12, 30, Width - 24, 20);
            p_FolderList.SetBounds(12, 62, Width - 24, Height - 132);
            p_ButtonAdd.SetBounds(12, Height - 58, x - 6, 20);
            p_ButtonRemove.SetBounds(12 + x, Height - 58, x, 20);
            p_ButtonSave.SetBounds(12, Height - 32, x - 6, 20);
            p_ButtonCancel.SetBounds(12 + x, Height - 32, x, 20);
        ...
        #region Child Events
        private void p_NameGroup_DefaultButtonClicked(object sender, EventArgs e)
        {
            p_NameGroup.TextBox.Text = "New Playlist";
        }
        
        private void p_NameGroup_TextChanged(object sender, EventArgs e)
        {
            p_ButtonSave.Enabled = (p_FolderList.Items.Count != 0 && !string.IsNullOrEmpty(p_NameGroup.TextBox.Text.Trim(new char[] { ' ' })));
        }

        private void p_FolderList_ItemsChanged(object sender, EventArgs e)
        {
            p_ButtonSave.Enabled = (p_FolderList.Items.Count != 0 && !string.IsNullOrEmpty(p_NameGroup.TextBox.Text.Trim(new char[] { ' ' })));
        }

        private void p_FolderList_SelectedItemChanged(object sender, EventArgs e)
        {
            p_ButtonRemove.Enabled = p_FolderList.SelectedItem != null;
        }

        private void p_ButtonAdd_MouseClick(object sender, MouseEventArgs e)
        {
            System.Windows.Forms.FolderBrowserDialog fbd = new System.Windows.Forms.FolderBrowserDialog();
            fbd.Description = "Add Folder";
            fbd.SelectedPath = Global.Settings.MusicFolder;
            fbd.ShowNewFolderButton = false;

            if (fbd.ShowDialog(this) == DialogResult.OK)
            {
                foreach (ListItem item in p_FolderList.Items)
                {
                    if (item.Text.ToLower().StartsWith(fbd.SelectedPath.ToLower()) || fbd.SelectedPath.ToLower().StartsWith(item.Text.ToLower()))
                    {
                        InfoDialog id = new InfoDialog("Folder or sub-folder(s) already found, please try again.");
                        id.ShowDialog(this);
                        id.Dispose();
                ...
                p_FolderList.Add(new ListItem(fbd.SelectedPath));
            }

            fbd.Dispose();
        }

        private void p_ButtonRemove_MouseClick(object sender, MouseEventArgs e)
        {
            p_FolderList.Remove(p_FolderList.SelectedItem);
            p_FolderList.SelectedItem = null;

            p_ButtonRemove.Enabled = false;
        }

        private void p_ButtonSave_MouseClick(object sender, MouseEventArgs e)
        {
            foreach (char c in Path.GetInvalidFileNameChars())
            {
                if (p_NameGroup.TextBox.Text.Contains(c.ToString()))
                {
                    InfoDialog id = new InfoDialog("Invalid playlist name, please try again.");
                    id.ShowDialog(this);
                    id.Dispose();
            ...
            foreach (LibraryItem item in Global.MainWindow.Library.Items)
            {
                if (item.Text.ToLower().Equals(p_NameGroup.TextBox.Text.ToLower()))
                {
                    InfoDialog id = new InfoDialog("Invalid playlist name, please try again.");
                    id.ShowDialog(this);
                    id.Dispose();
        ...
        private bool Save()
        {
            try
            {
                string url = Global.Settings.MusicFolder + "\\" + p_NameGroup.TextBox.Text + ".sp3";
                ...
                {
                    XmlElement element = xml.CreateElement("Folder");
                    element.SetAttribute("URL", item.Text);
                    element.SetAttribute("Include", "1");
                    root.AppendChild(element);
                }
    ...
    public class PlaylistItem : GenericListItem
    {
        ...
        public PlaylistItem(string url, string artist, string title, string album, string genre, int track, int trackCount, int year, string type, int playCount, int bitrate, int frequency, TimeSpan duration, int size)
        {
            p_Error = ErrorCodes.None;
            p_URL = url;
            p_Artist = artist;
            ...
            p_Genre = genre;
            p_Track = (short?)track;
            p_TrackCount = (short?)trackCount;
            p_Year = (short?)year;
            p_Rating = 0.0d;
            p_FileType = type;
            p_Encoder = string.Empty;
            p_PlayCount = playCount;
            p_LastPlayed = string.Empty;
            p_Mode = string.Empty;
            p_Layers = 0;
            p_Bitrate = bitrate;
    ...
    public class PluginList : GenericList<PluginListItem>
    {
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            PluginListItem item = e.Item as PluginListItem;
            if (item == null) return;
            ...
            if (item.IPlugin != null) textColor = Global.Skin.List.Play_TextColor;
            if (item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.Graphics.DrawString(item.Name, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);
            e.Graphics.DrawString(item.Description, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X + x, e.Bounds.Y, (x * 2) - 34, e.Bounds.Height), e.StringFormat);
            
            e.StringFormat.Alignment = StringAlignment.Far;
            if (item.URL.StartsWith("http://")) e.Graphics.DrawString("Trusted", Skin.WindowFont, new SolidBrush(Global.Skin.List.Play_TextColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 60, e.Bounds.Y, 60, e.Bounds.Height), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Near;
        }
    ...
    public class RefreshDialog : Dialog
    {
        ...
        //private List<LibraryItem> p_Playlists;
        ...
        //public List<LibraryItem> Playlists
        //{
            //get { return p_Playlists; }
        //}
        ...
        #region Constructor/Destructor
        public RefreshDialog(List<string> folders, string message) : base()
        {
            Sizable = false;
            ...
            p_CurrentLabel = new Label(this, null, "Current directory: ");
            ...
            //p_Playlists = new List<LibraryItem>();
            p_StartTime = 0;
            p_ElapsedTime = 0;
            p_DirectoryCount = 0;
            p_DirInfo = null;
        ...
        ~RefreshDialog()
        {
            ...
            //foreach (LibraryItem item in p_Playlists)
            //{
                //item.Dispose();
            //}
            //p_Playlists.Clear();
                ...
                foreach (string folder in p_Folders)
                {
                    ...
                    foreach (FileInfo fi in fileInfo)
                    {
                        ...
                        foreach (string ext in Global.Settings.MusicFilter.Split(new char[] { ';' }))
                        {
                            if (fi.FullName.ToLower().EndsWith(ext.ToLower()))
                            {
                                p_Files.Add(new PlaylistItem(fi.FullName, false));
                                break;
                            }
                        }
                        ...
                        //if (fi.FullName.ToLower().EndsWith(ext.ToLower()))
                        //{
                        //p_Playlists.Add(new LibraryItem(fi.FullName));
                        //break;
                        //}
                        //}
                    }
    ...
    public class Skin : GenericListItem
    {
        ...
        public void Delete()
        {
            try
            {
                ...
                string data = "method=3&name=" + HttpUtility.UrlEncode(p_Name) + "&author=" + Global.Steam.Client.GetSteamID().ToUInt64().ToString() + "&url=" + HttpUtility.UrlEncode(p_URL);
                
                wc.UploadString("http://steamp3.ta0soft.com/skins/skins.php", "POST", data);
                wc.Dispose();
            }
            catch { }
        }
    ...
    public class SkinDesigner : Dialog
    {
        ...
        private void p_SaveButton_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (id.ShowDialog(this) == DialogResult.Yes)
            {
                ...
                Global.MainWindow.SkinList.Refresh(); //?
    ...
    public class SkinList : GenericList<Skin>
    {
        ...
        private Menu p_ContextMenu;
        private WebClient p_WebClient;
        private SkinDesigner p_SkinDesigner;
        #endregion
        ...
        public SkinDesigner SkinDesigner
        {
            get { return p_SkinDesigner; }
        }
        ...
        public override void Dispose()
        {
            if (p_SkinDesigner != null) p_SkinDesigner.Dispose();
            p_WebClient.Dispose();
            p_ContextMenu.Dispose();
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            ...
            if (item.URL.Equals(Global.Skin.URL)) textColor = Global.Skin.List.Play_TextColor;
            if (item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.Graphics.DrawString(item.Name, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, x, e.Bounds.Height), e.StringFormat);
            e.Graphics.DrawString(item.AuthorPersonaName, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X + x, e.Bounds.Y, (x * 2) - 80, e.Bounds.Height), e.StringFormat);

            e.StringFormat.Alignment = StringAlignment.Far;
            e.Graphics.DrawString(item.Created, Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 80, e.Bounds.Y, 80, e.Bounds.Height), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Near;
        }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && Clicks > 1)
            {
                if (p_SkinDesigner == null || p_SkinDesigner.IsDisposed)
                {
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (p_SkinDesigner == null || p_SkinDesigner.IsDisposed)
                {
                    if (SelectedItem != null)
                    {
                        p_ContextMenu.AddRange(new MenuItem[] { new MenuItem(">", "Load Skin"), new MenuItem("¤", "Edit Skin...") });
                        if (SelectedItem.Author == Global.Steam.Client.GetSteamID().ToUInt64().ToString()) p_ContextMenu.Add(new MenuItem("x", "Delete Skin"));
                        p_ContextMenu.Add(new MenuItem(string.Empty, "-"));
                    }
                ...
                {
                    p_ContextMenu.AddRange(new MenuItem[] { new MenuItem("q", "Refresh Skins") });
                }
            ...
            switch (p_ContextMenu.SelectedItem.Text)
            {
                ...
                case "Edit Skin...":
                    if (p_SkinDesigner == null || p_SkinDesigner.IsDisposed)
                    {
                        ...
                        p_SkinDesigner = new SkinDesigner();
                        p_SkinDesigner.Show();
                ...
                case "Delete Skin":
                    if (p_SkinDesigner == null || p_SkinDesigner.IsDisposed)
                    {
                        ...
                        if (id.ShowDialog(Owner) == DialogResult.Yes)
                        {
                            UI.Skin skin = new UI.Skin(SelectedItem.URL); //?
                            skin.Delete();
        ...
        private void p_WebClient_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            if (e.Error == null)
            {
                try
                {
                    ...
                    Global.MainWindow.AchievementList.IncreaseSkinnyDipper(); //?
    ...
    public class Station : GenericListItem
    {
        ...
        public string StreamURL
        {
            get
            {
                ...
                {
                    if (p_URL.ToLower().Contains(".pls")) p_StreamURL = Global.GetIniValueFromString(PlaylistData, "File1", string.Empty);
        ...
        #region Constructor/Destructor
        public Station(string url, string name, string genre, bool local) : base()
        {
            p_URL = url;
            p_Name = name;
            p_Genre = genre;
    ...
    public class StationDialog : Dialog
    {
        ...
        private void p_AutoButton_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                p_NameGroup.TextBox.Text = Global.GetIniValueFromString(result, "Title1", string.Empty);

                p_Shoutcast = new Shoutcast(Global.GetIniValueFromString(result, "File1", string.Empty), 15000);
    ...
    public class StationList : GenericList<Station>
    {
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            Station item = e.Item as Station;
            if (item == null) return;
            ...
            e.Graphics.DrawString(item.Name, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, e.Bounds.Width - 110, e.Bounds.Height), e.StringFormat);

            e.StringFormat.Alignment = StringAlignment.Far;
            e.Graphics.DrawString(item.Genre, Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 110, e.Bounds.Y, 110, e.Bounds.Height), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Near;
        }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && Clicks > 1)
            {
                if (Global.Settings.Notifications) Global.MediaPlayer.Stream(SelectedItem, "Station", true);
                else Global.MediaPlayer.Stream(SelectedItem, string.Empty, true);
            }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (SelectedItem != null)
                {
                    p_ContextMenu.Add(new MenuItem("4", "Stream Station"));
                    if (SelectedItem.Local) p_ContextMenu.Add(new MenuItem("x", "Delete Station"));
                    p_ContextMenu.Add(new MenuItem(string.Empty, "-"));
                }
            ...
            switch (p_ContextMenu.SelectedItem.Text)
            {
                ...
                case "Add Station...":
                    ...
                    if (sd.ShowDialog(Owner) == DialogResult.OK)
                    {
                        ...
                        Global.Settings.RadioStations.Add(new Station(sd.URLGroup.TextBox.Text, sd.NameGroup.TextBox.Text, sd.GenreGroup.DropDown.Text, true));
        ...
        private void p_WebClient_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            if (e.Error == null)
            {
                try
                {
                    ...
                    foreach (XmlNode node in nodeList)
                    {
                        list.Add(new Station(Global.GetXmlValue(node, "URL", string.Empty), Global.GetXmlValue(node, "Name", "N/A"), Global.GetXmlValue(node, "Genre", "N/A"), false));
                    }
    ...
    public class Tab : GenericListItem
    {
        ...
        #region Constructor/Destructor
        public Tab(string icon, string text) : base()
        {
            Text = text;
        ...
        {
            p_TabWidth = TextRenderer.MeasureText(Text, Skin.WindowFont).Width + 22;
    ...
    public class TabContainer : GenericList<Tab>
    {
        ...
        protected override void DrawItems(Graphics g, Rectangle bounds)
        {
            ...
            for (int i = FirstIndex; i < FilteredItems.Count; i++)
            {
                ...
                if (FilteredItems[i].TabWidth > 30) g.DrawString(FilteredItems[i].Text, Skin.WindowFont, new SolidBrush(textColor), new RectangleF((bounds.X + x) + 18, bounds.Y, FilteredItems[i].TabWidth - 24, 22), sf);
    ...
    public class ToolTip : Popup
    {
        ...
        public string Message
        {
            ...
            set
            {
                ...
                Size s = TextRenderer.MeasureText(p_Message, Skin.WindowFont);
                ...
                Size = new Size(s.Width + offset, 18);
        ...
        protected override void OnDraw(Graphics g)
        {
            ...
            //TextRenderer.DrawText(g, p_Message, Skin.WindowFont, new Rectangle(2, 0, Width - 4, Height), Global.Skin.Popup.TextColor, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPadding);
    ...
    public class UpdateList : GenericList<UpdateListItem>
    {
        ...
        protected override void DrawItem(DrawItemEventArgs e)
        {
            UpdateListItem item = e.Item as UpdateListItem;
            if (item == null) return;

            Color textColor = Global.Skin.List.TextColor;

            if (item.Downloaded) textColor = Global.Skin.List.Play_TextColor;
            //if (item.Equals(HotItem)) textColor = Global.Skin.List.Over_TextColor;

            e.StringFormat.Alignment = StringAlignment.Near;
            e.Graphics.DrawString(item.Description, Skin.WindowFont, new SolidBrush(textColor), new RectangleF(e.Bounds.X, e.Bounds.Y, e.Bounds.Width - 110, e.Bounds.Height / 2), e.StringFormat);
            e.Graphics.DrawString(item.URL, Skin.WindowFont, new SolidBrush(Color.FromArgb(100, Global.Skin.List.TextColor)), new RectangleF(e.Bounds.X, e.Bounds.Y + (e.Bounds.Height / 2), e.Bounds.Width - 110, e.Bounds.Height / 2), e.StringFormat);
            e.StringFormat.Alignment = StringAlignment.Far;

            if (item.BytesRead > 0)
            {
                if (item.Downloaded) e.Graphics.DrawString("Installed", Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 120, e.Bounds.Y, 120, e.Bounds.Height / 2), e.StringFormat);
                else e.Graphics.DrawString(Global.ConvertBytes(item.BytesRead) + " / " + Global.ConvertBytes(item.Size), Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 120, e.Bounds.Y, 120, e.Bounds.Height / 2), e.StringFormat);

                int percent = Global.GetPercent(item.BytesRead, item.Size, 100);
                if (percent == 0) percent = 1;

                Rectangle r = new Rectangle((e.Bounds.X + e.Bounds.Width) - 100, e.Bounds.Y + (e.Bounds.Height / 2), percent, 16);
                Global.FillRoundedRectangle(e.Graphics, r, new LinearGradientBrush(r, Global.Skin.List.Play_BGColor, Global.Skin.List.Play_TextColor, LinearGradientMode.Horizontal));
            }
            else e.Graphics.DrawString(Global.ConvertBytes(item.Size), Skin.WindowFont, new SolidBrush(textColor), new RectangleF((e.Bounds.X + e.Bounds.Width) - 120, e.Bounds.Y, 120, e.Bounds.Height / 2), e.StringFormat);

            Global.DrawRoundedRectangle(e.Graphics, new Rectangle((e.Bounds.X + e.Bounds.Width) - 100, e.Bounds.Y + (e.Bounds.Height / 2), 100, 16), new Pen(Global.Skin.Slider.BorderColor));
        }
        #endregion
    }
    #endregion
    ...
    public class Window : Form
    {
        ...
        private void DrawBackground(Graphics g, Rectangle bounds)
        {
            ...
            if (!string.IsNullOrEmpty(Text) && p_DrawText)
            {
                ...
                else g.DrawString(Text, Skin.WindowFont, new SolidBrush(Global.Skin.Window.TextColor), new RectangleF(bounds.X + 5, bounds.Y + 5, bounds.Width - 46, 20), sf);
            }
