...
namespace Steamp3.SteamAPI
{
    ...
    public class SteamClient
    {
        ...
        public Bitmap GetFriendAvatar(SteamID friendID)
        {
            ...
            try
            {
                int avatarId = p_ClientFriends001.GetSmallFriendAvatar(friendID.ToUInt64());
                if (avatarId == 0) return null;

                int width = 32;
                int height = 32;
                byte[] imgData = new byte[4 * width * height];
                bool result = p_SteamUtils004.GetImageRGBA(avatarId, imgData, imgData.Length);
    ...
    public class ChatRoom
    {
        ...
        public ChatRoom(SteamClient client)
        {
            ...
            p_GetChatEntry = p_VTable.GetFunc<NativeGetChatRoomEntry>(101);
            p_GetChatName = p_VTable.GetFunc<NativeGetChatRoomName>(119);
        }
